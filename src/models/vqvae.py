...
class VQVAELightningSingle(L.LightningModule):
    ...
    def model_step(self, batch, return_x=False):
        ...
        if self.data_type == "hit":
            ...
            if aug_weight != 0:
                ...
                mask_augmented = batch["mask_augmented"]
                _, _, z_embed_augmented = self.model(None, x_hit_augmented, mask_augmented)
              
          

            

            x_hit_reco, vq_out, z_embed = self.model(None, x_hit, mask) # batch not used



            reco_loss = reco_loss_function(x_hit, x_hit_reco, mask)
            loss += reco_loss

            x1 = F.normalize(x_hit, dim=-1, eps=1e-8)       # [2, 7491, 4]
            x2 = F.normalize(x_hit_reco, dim=-1, eps=1e-8)  # [2, 7491, 4]
            cos_sim = (
                ((x1 * x2).sum(dim=-1) * mask).sum()
            ...
            if aug_weight != 0:
                aug_loss = CLIP_loss(z_embed, z_embed_augmented, mask, mask_augmented)
                
                loss += aug_weight * aug_loss
                
                loss_dict["aug_loss"] = aug_loss
            ...
            if return_x:
                return loss_dict, x_hit, x_hit_reco, mask, labels, code_idx
